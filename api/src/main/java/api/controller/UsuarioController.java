...
public class UsuarioController {
    ...
    public ResponseEntity<UsuarioResponseDto> cadastrar(@Valid @RequestBody UsuarioCadastroDto cadastroDto) {
        ...
        //int perfilId = 2;
        //cadastroDto.setPerfilId(perfilId);
        Usuario usuarioCadastrado = usuarioService.cadastrar(usuario, cadastroDto.getPerfil());
    ...
    @GetMapping("/{id}")
	@PreAuthorize("hasAuthority('Administrador') OR ( hasAuthority('Cliente') AND #id == authentication.principal.id)")
    ...
    @PatchMapping("/{id}")
	@PreAuthorize("hasAnyAuthority('Administrador', 'Cliente') AND (#id == authentication.principal.id)")
	...
	@Operation(summary = "Listar todos os usuários cadastrados", description = "Requisição exige um Bearer Token. Acesso restrito a Administrador",
